...
Provider rankings come from a continuously-running benchmark suite. Each provider/model is scored per `language` on three axes:
...
`GET /v1/routing/preview?language=en-US&optimize_for=accuracy` returns the same `SelectedCandidate` shape the router would use, including `runnersUp[].score`. Use it to debug "why did Speko pick X for this intent?".
