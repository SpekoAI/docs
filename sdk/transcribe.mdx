...
Transcribe an audio payload. The router picks the best STT provider for your `(language, vertical, optimizeFor)` and fails over automatically.
...
- `X-Speko-Intent`: JSON-serialized `{ language, vertical, optimizeFor? }`.
