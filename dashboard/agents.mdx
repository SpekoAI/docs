...
- **Stack preferences** *(optional)* — per-layer allowlists (STT / LLM / TTS). Empty layers = no constraint. See [Concepts → Agents → Stack preferences](/concepts/agents#stack-preferences) for the merge semantics with per-call overrides.
