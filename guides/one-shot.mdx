...
const t = await speko.transcribe(buf, { language: 'en-US', vertical: 'general' });
const a = await speko.synthesize('Hello', { language: 'en', vertical: 'general' });
...
t = speko.transcribe(buf, language="en-US", vertical="general")
a = speko.synthesize("Hello", language="en", vertical="general")
c = speko.complete(
    messages=[{"role": "user", "content": "Hi!"}],
    intent={"language": "en", "vertical": "general"},
)
